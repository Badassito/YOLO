...
This v6.1.0 specification-aligned script:
  ...
  - uses RAM/ZRAM more aggressively for 3D interpolation / 3D void-filling workspaces when enough memory+swap is available
...
def _copy_volume_to_oriented_memmap(
    ...
    if orientation == 'sagittal':
        oriented_mm = np.memmap(out_path, dtype=np.uint8, mode='w+', shape=(h_dim, t_dim, w_dim))
        ...
        oriented_mm = np.memmap(out_path, dtype=np.uint8, mode='w+', shape=(w_dim, t_dim, h_dim))
...
def interpolate_radial_view_pass_inplace(
    ...
    try:
        for orientation in ('sagittal', 'coronal'):
            ...
            oriented_mm = _copy_volume_to_oriented_memmap(mask_mm, orientation, oriented_path)
            try:
        ...
        for pass_idx in range(1, total_passes + 1):
            ...
            for model_name in sorted(view_volumes_by_model.keys()):
                for view in views:
                    ...
                    if view.family == 'radial':
                        stats = interpolate_radial_view_pass_inplace(
                            mask_mm=current_mm,
