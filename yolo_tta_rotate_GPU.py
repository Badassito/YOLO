...
def get_combined_mask(results, shape):
    """
    Optimized: Collapses masks on GPU first, then resizes once.
    """
    h, w = shape[:2]
    ...
    # 1. Keep data on GPU (results[0].masks.data is a PyTorch tensor)
    # Shape is (N, H_mask, W_mask)
    masks_tensor = results[0].masks.data

    if masks_tensor.shape[0] == 0:
        return np.zeros((h, w), dtype=np.uint8)

    # 2. Vectorized Merge on GPU
    # Check if any object exists at each pixel (Logical OR across dim 0)
    # We use > 0.5 to threshold probability masks if they aren't already binary
    combined_tensor = torch.any(masks_tensor > 0.5, dim=0)

    # 3. Move single merged mask to CPU
    # Convert bool -> uint8 (byte) -> numpy
    combined = combined_tensor.byte().cpu().numpy()

    # 4. Resize ONCE (if necessary)
    # Note: Use INTER_NEAREST for binary masks (faster and keeps edges sharp)
    if combined.shape[:2] != (h, w):
        combined = cv2.resize(combined, (w, h), interpolation=cv2.INTER_NEAREST)

    return combined

def run_tta_inference_batched(model, frame, conf, retina_masks=True):
    """Run TTA inference with all rotations batched together."""
    angles = [0, 90, 180, 270]
    h, w = frame.shape[:2]

    # Create all rotated versions at once
    rotated_frames = [
        frame if angle == 0 else rotate_image(frame, angle)
        for angle in angles
    ]
    ...
    for angle, result in zip(angles, results):
        # Get shape of the rotated frame for this result
        rot_shape = rotated_frames[angles.index(angle)].shape
        mask = get_combined_mask([result], rot_shape)
...
def process_video(model_path, source, conf, imgsz, output_path, temporal_n=0,
    ...
    while True:
        ...
        mask = run_tta_inference_batched(model, frame, conf, retina_masks=True)
