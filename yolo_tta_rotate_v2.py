"""
YOLO Segmentation with Test Time Augmentation (TTA) using arbitrary rotation angles,
then merges masks using OR operation. Optional temporal smoothing using ± N frames.
...
def rotate_image(image, angle):
    """
    Rotate image by arbitrary angle around center.
    Maintains original dimensions, padding empty regions with black.
    """
    if angle == 0:
        return image
    ...
    # Get rotation matrix (positive angle = counter-clockwise in OpenCV)
    M = cv2.getRotationMatrix2D(center, angle, 1.0)

    # Apply rotation with black padding
...
def rotate_mask_back(mask, angle):
    ...
    """
    if mask is None or angle == 0:
        return mask

    h, w = mask.shape[:2]
    center = (w / 2, h / 2)

    # Rotate by negative angle to reverse
    M = cv2.getRotationMatrix2D(center, -angle, 1.0)

    rotated = cv2.warpAffine(
        mask, M, (w, h),
        flags=cv2.INTER_NEAREST,  # Preserve binary mask values
        borderMode=cv2.BORDER_CONSTANT,
        borderValue=0
    )
...
def run_tta_inference(model, frame, conf, angles, device, retina_masks=True):
    """Run TTA inference with arbitrary rotations as a single batch and merge masks."""
    h, w = frame.shape[:2]

    # Pre-rotate all images
    rotated_images = [rotate_image(frame, angle) for angle in angles]

    # Run batch inference (single call for all rotations)
    results = model.predict(
        ...
        device=device,
        half=True,
        batch=len(angles)  # explicitly set batch size
    )

    # Process results and rotate masks back
    merged_mask = np.zeros((h, w), dtype=np.uint8)

    for result, angle in zip(results, angles):
        # get_combined_mask expects a list, so wrap single result
        mask = get_combined_mask([result], frame.shape)
...
def parse_angles(angles_str):
    ...
    Examples: "0,45,90,135" or "0,90,180,270"
    """
    return [float(a.strip()) for a in angles_str.split(',')]


def process_video(model_path, source, conf, imgsz, output_path, angles, device,
                  temporal_n=0, save_txt=False, save_binary=False):
    ...
    print(f"TTA angles: {angles} (OR merge)")
    if temporal_n > 0:
    ...
    while True:
        ...
        mask = run_tta_inference(model, frame, conf, angles, device, retina_masks=True)
...
def main():
    parser = argparse.ArgumentParser(
        description='YOLO Segmentation with Arbitrary Rotation TTA',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Standard 90° rotations
  python %(prog)s --model best.pt --source video.mp4 --output out.avi --angles 0,90,180,270

  # 45° increments (8 angles)
  python %(prog)s --model best.pt --source video.mp4 --output out.avi --angles 0,45,90,135,180,225,270,315

  # Just 45° offset
  python %(prog)s --model best.pt --source video.mp4 --output out.avi --angles 0,45
        """
    )
    ...
    parser.add_argument('--device', type=str, default='0',
                        help='Device for inference, e.g., 0, 1, cpu (default: 0)')
