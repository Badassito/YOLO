...
This v13.0.0_SLURM implementation of the v13.0.0_SLURM specification, based on the v12.2.15 scheduling/native-volume rework plus the v12.2.1-v12.2.14 performance patches:
  - v13.0.0 derives an approximate-cube virtual working geometry by default while rendering YOLO views directly from the decoded gray8 volume; the legacy materialized cube path remains available only with YOLO_TTA_PROCESSING_VOLUME_MODE=cube or YOLO_TTA_FORCE_PROCESSING_CUBE=1.
  - v13.0.0 keeps unbounded streaming prediction-source creation by default, decouples cheap source/ref creation from CPU rendering, removes the old fixed four-source warmup/staging default, and adds explicit multi-GPU inference-worker scheduling so each GPU receives single-device YOLO work.
  ...
  - v12.2.8 adds a dedicated single --angle streaming cleanup path: when exactly one augmentation angle is requested, min_conf filtering, view-native per-slice min_radius filtering, and then 2D hole filling are applied as YOLO slices stream in; only true volume-level cleanup waits for the completed view volume
  ...
  - fuses per-slice cleanup work where the slice orientation matches the required semantics
    (notably min_conf filtering, min_radius where applicable, and 2D hole filling in v13 order)
  - overlaps GPU inference with CPU-side view interpolation, consolidated-tile interpolation, and output writing
  - exposes --retina_mask_processor cpu|gpu: CPU mode passes retina_masks=False to YOLO and reconstructs retina-quality masks from prototypes/coefficients on CPU; GPU mode passes retina_masks=True to YOLO
...
# v13.0.0 removes skeletonization from the pipeline and from default dependencies.
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--device", default="0", type=str, help="Device(s) for YOLO inference: one GPU index, comma-separated GPU indices, or cpu. GPU indices and cpu cannot be mixed")
    p.add_argument("--retina_mask_processor", default=None, choices=("cpu", "gpu"), type=str,
                   help="Resolve full-resolution retina masks on cpu or gpu. Defaults to cpu for single-device GPU/CPU runs, gpu for multi-GPU runs unless explicitly overridden; device=cpu forces cpu")
...
def processing_volume_mode() -> str:
    ...
    v13.0.0 defaults to ``virtual_cube``: derive the approximate-cube working
    dimensions required by the specification, but render every view directly from
    the decoded native gray8 volume through a single reslice/scale transform.  The
    old materialized cube path remains available only for regression/comparison by
    setting ``YOLO_TTA_PROCESSING_VOLUME_MODE=cube`` or
    ``YOLO_TTA_FORCE_PROCESSING_CUBE=1``.
    """
    ...
    aliases = {
        '': 'virtual_cube',
        'virtual': 'virtual_cube',
        'virtual_cube': 'virtual_cube',
        'virtual-cube': 'virtual_cube',
        'v13': 'virtual_cube',
        'v13.0.0': 'virtual_cube',
        'native': 'native',
        'decoded': 'native',
        'input': 'native',
        'none': 'native',
        ...
        'cube': 'cube',
        'cubic': 'cube',
        'materialized_cube': 'cube',
        'materialized-cube': 'cube',
        'legacy': 'cube',
        'legacy_cube': 'cube',
        'v12': 'cube',
        'v12.2.0': 'cube',
    ...
    if mode is None:
        print(f"Warning: unsupported YOLO_TTA_PROCESSING_VOLUME_MODE={raw!r}; using v13 virtual_cube geometry.")
        return 'virtual_cube'
    return str(mode)
...
def _render_tilted_array_on_grid(
    ...
    *,
    ...
    for y0 in range(0, int(grid_h), int(block_rows)):
        ...
        u_idx = np.asarray(plan.x_idx[y0:y1], dtype=np.int32)  # base-view horizontal axis in virtual coordinates
        v_idx = np.asarray(plan.y_idx[y0:y1], dtype=np.int32)  # base-view vertical axis in virtual coordinates

        source_shape = tuple(int(x) for x in np.asarray(volume_arr).shape[:3])
        virtual_shape = _view_virtual_shape_tyx(view) if '_view_virtual_shape_tyx' in globals() else source_shape
        if source_shape != virtual_shape:
            src_t, src_h, src_w = source_shape
            if base_view == 'transverse':
                zf = _map_virtual_coord_to_source(stack_src, int(view.full_t), int(src_t))
                yf = _map_virtual_coord_to_source(v_idx, int(view.full_h), int(src_h))
                xf = _map_virtual_coord_to_source(u_idx, int(view.full_w), int(src_w))
            elif base_view == 'sagittal':
                zf = _map_virtual_coord_to_source(v_idx, int(view.full_t), int(src_t))
                yf = _map_virtual_coord_to_source(stack_src, int(view.full_h), int(src_h))
                xf = _map_virtual_coord_to_source(u_idx, int(view.full_w), int(src_w))
            elif base_view == 'coronal':
                zf = _map_virtual_coord_to_source(v_idx, int(view.full_t), int(src_t))
                yf = _map_virtual_coord_to_source(u_idx, int(view.full_h), int(src_h))
                xf = _map_virtual_coord_to_source(stack_src, int(view.full_w), int(src_w))
            else:  # pragma: no cover
                raise ValueError(f'Unsupported Tilted View base: {base_view}')
            rendered = _sample_volume_trilinear_block(
                volume_arr,
                np.asarray(zf, dtype=np.float32),
                np.asarray(yf, dtype=np.float32),
                np.asarray(xf, dtype=np.float32),
                valid,
                mask_mode=bool(mask_mode),
            )
            out_block = out[y0:y1]
...
def _view_virtual_shape_tyx(view: ViewInfo) -> Tuple[int, int, int]:
    return (int(view.full_t), int(view.full_h), int(view.full_w))


def _source_matches_view_virtual_geometry(volume_arr: np.ndarray, view: ViewInfo) -> bool:
    try:
        return tuple(int(x) for x in np.asarray(volume_arr).shape[:3]) == _view_virtual_shape_tyx(view)
    except Exception:
        return False


def _map_virtual_coord_to_source(coord: np.ndarray | float | int, virtual_count: int, source_count: int) -> np.ndarray | float:
    if int(virtual_count) <= 1 or int(source_count) <= 1:
        if isinstance(coord, np.ndarray):
            return np.zeros_like(coord, dtype=np.float32)
        return 0.0
    scale = float(source_count - 1) / float(virtual_count - 1)
    if isinstance(coord, np.ndarray):
        return np.asarray(coord, dtype=np.float32) * np.float32(scale)
    return float(coord) * scale


def _blend_resize_source_t_slice(volume_arr: np.ndarray, z_float: float, out_h: int, out_w: int) -> np.ndarray:
    src_t = int(volume_arr.shape[0])
    z0 = int(math.floor(float(z_float)))
    z1 = min(src_t - 1, z0 + 1)
    alpha = float(z_float - float(z0))
    wait_for_volume_slice_ready(volume_arr, int(z0))
    f0 = _resize_gray_slice_nearest_or_linear(np.asarray(volume_arr[int(z0)]), int(out_w), int(out_h), cv2.INTER_LINEAR)
    if z1 == z0 or alpha <= 1e-7:
        return np.ascontiguousarray(f0, dtype=np.uint8)
    wait_for_volume_slice_ready(volume_arr, int(z1))
    f1 = _resize_gray_slice_nearest_or_linear(np.asarray(volume_arr[int(z1)]), int(out_w), int(out_h), cv2.INTER_LINEAR)
    return np.ascontiguousarray(cv2.addWeighted(f0, float(1.0 - alpha), f1, float(alpha), 0.0), dtype=np.uint8)


def _blend_resize_source_axis_plane(volume_arr: np.ndarray, axis: str, axis_float: float, out_h: int, out_w: int) -> np.ndarray:
    wait_for_volume_ready(volume_arr)
    src_t, src_h, src_w = (int(volume_arr.shape[0]), int(volume_arr.shape[1]), int(volume_arr.shape[2]))
    if axis == 'y':
        max_axis = src_h
    elif axis == 'x':
        max_axis = src_w
    else:
        raise ValueError(f'Unsupported virtual axis plane {axis!r}')
    a0 = int(math.floor(float(axis_float)))
    a1 = min(max_axis - 1, a0 + 1)
    alpha = float(axis_float - float(a0))
    if axis == 'y':
        p0 = np.ascontiguousarray(volume_arr[:, int(a0), :])
        p1 = np.ascontiguousarray(volume_arr[:, int(a1), :]) if a1 != a0 and alpha > 1e-7 else None
    else:
        p0 = np.ascontiguousarray(volume_arr[:, :, int(a0)])
        p1 = np.ascontiguousarray(volume_arr[:, :, int(a1)]) if a1 != a0 and alpha > 1e-7 else None
    r0 = _resize_gray_slice_nearest_or_linear(p0, int(out_w), int(out_h), cv2.INTER_LINEAR)
    if p1 is None:
        return np.ascontiguousarray(r0, dtype=np.uint8)
    r1 = _resize_gray_slice_nearest_or_linear(p1, int(out_w), int(out_h), cv2.INTER_LINEAR)
    return np.ascontiguousarray(cv2.addWeighted(r0, float(1.0 - alpha), r1, float(alpha), 0.0), dtype=np.uint8)


def _source_radial_view_for_virtual_view(radial_view: ViewInfo, source_shape: Tuple[int, int, int]) -> ViewInfo:
    src_t, src_h, src_w = (int(source_shape[0]), int(source_shape[1]), int(source_shape[2]))
    diameter = int(min(src_w, src_h))
    return ViewInfo(
        name='radial',
        num_slices=int(radial_view.num_slices),
        src_h=src_t,
        src_w=diameter,
        pad_mode='pad',
        family='radial',
        summary_family='radial',
        display_name='Radial source-native sampler',
        full_t=src_t,
        full_h=src_h,
        full_w=src_w,
        azimuths_deg=tuple(float(a) for a in radial_view.azimuths_deg),
        diameter=diameter,
        center_x=float((src_w - 1) / 2.0),
        center_y=float((src_h - 1) / 2.0),
        roi_radius=float(max(0.0, (diameter - 1) / 2.0)),
        horizontal_axis='r',
        vertical_axis='t',
        stack_axis='azimuth',
...
def _render_virtual_radial_frame(volume_arr: np.ndarray, view: ViewInfo, index: int) -> np.ndarray:
    wait_for_volume_ready(volume_arr)
    source_view = _source_radial_view_for_virtual_view(view, tuple(int(x) for x in volume_arr.shape[:3]))
    angle_deg = float(view.azimuths_deg[int(index)])
    sampler = get_radial_sampler(source_view, angle_deg)
    native_radial = np.ascontiguousarray(extract_radial_slice_frame(volume_arr, sampler))
    if int(native_radial.shape[0]) == int(view.src_h) and int(native_radial.shape[1]) == int(view.src_w):
        return native_radial
    return _resize_gray_slice_nearest_or_linear(native_radial, int(view.src_w), int(view.src_h), cv2.INTER_LINEAR)


def _sample_volume_trilinear_block(
    volume_arr: np.ndarray,
    z_float: np.ndarray,
    y_float: np.ndarray,
    x_float: np.ndarray,
    ...
    *,
    ...
    shape = tuple(int(x) for x in z_float.shape)
    out = np.zeros(shape, dtype=np.uint8)
    valid_bool = np.asarray(valid, dtype=bool)
    if not np.any(valid_bool):
        return out
    src_t, src_h, src_w = (int(volume_arr.shape[0]), int(volume_arr.shape[1]), int(volume_arr.shape[2]))
    z = np.asarray(z_float[valid_bool], dtype=np.float32)
    y = np.asarray(y_float[valid_bool], dtype=np.float32)
    x = np.asarray(x_float[valid_bool], dtype=np.float32)
    z0 = np.floor(z).astype(np.int32, copy=False); y0 = np.floor(y).astype(np.int32, copy=False); x0 = np.floor(x).astype(np.int32, copy=False)
    z0 = np.clip(z0, 0, src_t - 1); y0 = np.clip(y0, 0, src_h - 1); x0 = np.clip(x0, 0, src_w - 1)
    z1 = np.minimum(z0 + 1, src_t - 1).astype(np.int32, copy=False)
    y1 = np.minimum(y0 + 1, src_h - 1).astype(np.int32, copy=False)
    x1 = np.minimum(x0 + 1, src_w - 1).astype(np.int32, copy=False)
    dz = (z - z0.astype(np.float32, copy=False)).astype(np.float32, copy=False)
    dy = (y - y0.astype(np.float32, copy=False)).astype(np.float32, copy=False)
    dx = (x - x0.astype(np.float32, copy=False)).astype(np.float32, copy=False)

    c000 = np.asarray(volume_arr[z0, y0, x0], dtype=np.float32)
    c001 = np.asarray(volume_arr[z0, y0, x1], dtype=np.float32)
    c010 = np.asarray(volume_arr[z0, y1, x0], dtype=np.float32)
    c011 = np.asarray(volume_arr[z0, y1, x1], dtype=np.float32)
    c100 = np.asarray(volume_arr[z1, y0, x0], dtype=np.float32)
    c101 = np.asarray(volume_arr[z1, y0, x1], dtype=np.float32)
    c110 = np.asarray(volume_arr[z1, y1, x0], dtype=np.float32)
    c111 = np.asarray(volume_arr[z1, y1, x1], dtype=np.float32)
    ...
    vals = c0 * (1.0 - dz) + c1 * dz
    out_vals = (vals >= 0.5).astype(np.uint8, copy=False) if bool(mask_mode) else np.clip(np.rint(vals), 0.0, 255.0).astype(np.uint8, copy=False)
    out[valid_bool] = out_vals
    return out
...
def iter_view_frames(
    ...
    """Yield single-channel frames for a view, in virtual working slice order."""
    if view_frames is not None:
        for idx in range(int(view.num_slices)):
            yield np.asarray(view_frames[int(idx)])
        return
    for idx in range(int(view.num_slices)):
        yield get_view_frame_by_index(volume_rgb, view, int(idx), view_frames=None)
...
def get_view_frame_by_index(
    ...
    src_t, src_h, src_w = (int(volume_rgb.shape[0]), int(volume_rgb.shape[1]), int(volume_rgb.shape[2]))
    virtual_t, virtual_h, virtual_w = _view_virtual_shape_tyx(view)
    geometry_matches = _source_matches_view_virtual_geometry(volume_rgb, view)

    if view.name == 'transverse':
        if geometry_matches:
            wait_for_volume_slice_ready(volume_rgb, int(index))
            return np.asarray(volume_rgb[int(index)])
        src_z = _linear_source_index(int(index), int(virtual_t), int(src_t))
        return _blend_resize_source_t_slice(volume_rgb, float(src_z), int(virtual_h), int(virtual_w))

    if view.name == 'sagittal':
        if geometry_matches:
            wait_for_volume_ready(volume_rgb)
            return np.ascontiguousarray(volume_rgb[:, int(index), :])
        src_y = _linear_source_index(int(index), int(virtual_h), int(src_h))
        return _blend_resize_source_axis_plane(volume_rgb, 'y', float(src_y), int(virtual_t), int(virtual_w))

    if view.name == 'coronal':
        if geometry_matches:
            wait_for_volume_ready(volume_rgb)
            return np.ascontiguousarray(volume_rgb[:, :, int(index)])
        src_x = _linear_source_index(int(index), int(virtual_w), int(src_w))
        return _blend_resize_source_axis_plane(volume_rgb, 'x', float(src_x), int(virtual_t), int(virtual_h))
    ...
    if view.name == 'radial':
        ...
        return _render_virtual_radial_frame(volume_rgb, view, int(index))
...
class DevicePlan:
    ...
    def primary_device(self) -> str:
        return self.inference_devices[0] if self.inference_devices else 'cpu'

    @property
    def multi_gpu(self) -> bool:
        return (not self.is_cpu) and len(self.inference_devices) > 1


def resolve_device_plan(device: str) -> DevicePlan:
    raw = str(device or '0').strip() or '0'
    tokens = [tok.strip() for tok in re.split(r'[,\s]+', raw) if tok.strip()]
    if not tokens:
        tokens = ['0']
    lows = [tok.lower() for tok in tokens]
    has_cpu = any(tok == 'cpu' for tok in lows)
    has_gpu = any(tok != 'cpu' for tok in lows)
    if has_cpu and has_gpu:
        raise ValueError('--device may contain GPU indices or cpu, but cannot mix GPU indices and cpu')
    if has_cpu:
        if len(tokens) != 1:
            raise ValueError('--device cpu cannot be combined with any other device token')
        return DevicePlan(raw=raw, inference_devices=('cpu',), is_cpu=True)
    ...
    for tok in tokens:
        ...
        if low.startswith('cuda:'):
            dev = tok
        elif low.startswith('cuda') and low != 'cuda':
            dev = tok
        elif tok.isdigit():
            dev = tok
        else:
            raise ValueError(f'Unsupported --device token {tok!r}; expected cpu or GPU indices like 0,1,2')
        key = canonical_single_device(dev)
        if key in seen:
            continue
        seen.add(key)
        devices.append(dev)
    if not devices:
        devices = ['0']
    return DevicePlan(raw=raw, inference_devices=tuple(devices), is_cpu=False)


def resolve_retina_mask_processor_arg(requested: Optional[str], device_plan: DevicePlan) -> str:
    if device_plan.is_cpu:
        if requested and str(requested).strip().lower() == 'gpu':
            print('Warning: --device cpu forces --retina_mask_processor cpu; ignoring explicit gpu value.')
        return 'cpu'

    if requested is not None:
        proc = str(requested).strip().lower()
        if proc not in {'cpu', 'gpu'}:
            raise ValueError('--retina_mask_processor must be cpu or gpu')
        return proc

    env_proc = os.environ.get('YOLO_TTA_RETINA_MASK_PROCESSOR', '').strip().lower()
    if env_proc in {'cpu', 'gpu'}:
        return env_proc
    if os.environ.get('YOLO_TTA_CPU_RETINA_MASKS') is not None:
        return 'cpu' if _env_flag('YOLO_TTA_CPU_RETINA_MASKS', True) else 'gpu'

    # v13.0.0 default: CPU for one GPU, GPU for multiple GPUs so CPU-side mask
    # reconstruction does not become the throughput limiter.
    return 'gpu' if device_plan.multi_gpu else 'cpu'


def make_predict_config_for_device(base_cfg: 'PredictConfig', device: str) -> 'PredictConfig':
    return PredictConfig(
        imgsz=int(base_cfg.imgsz),
        conf=float(base_cfg.conf),
        device=str(device),
        half=bool(base_cfg.half),
        int8=bool(base_cfg.int8),
        batch=max(1, int(base_cfg.batch)),
    )
...
_RETINA_MASK_PROCESSOR = 'cpu'


def set_retina_mask_processor(processor: str) -> None:
    global _RETINA_MASK_PROCESSOR
    proc = str(processor or 'cpu').strip().lower()
    if proc not in {'cpu', 'gpu'}:
        raise ValueError('--retina_mask_processor must be cpu or gpu')
    _RETINA_MASK_PROCESSOR = proc


def retina_mask_processor() -> str:
    return str(_RETINA_MASK_PROCESSOR)


def cpu_retina_masks_enabled() -> bool:
    """Return True when v13 CPU retina-mask reconstruction is active.

    CPU mode passes ``retina_masks=False`` to Ultralytics, captures compact
    prototype/coefficient/box/confidence tensors, moves them to CPU, and reconstructs
    bbox-local retina-quality binary masks in prediction-result workers. GPU mode
    preserves Ultralytics native ``retina_masks=True`` behavior.
    """
    return retina_mask_processor() == 'cpu'
...
def predict_source_and_accumulate(
    ...
    *,
    ...
    streaming_cleanup_fill_holes: bool = True,
    ...
    stream_fill_holes = bool(streaming_cleanup_fill_holes)
    ...
    def _process_prediction_unit(idx_i: int, masks_obj: Optional[object], confs_arr: Optional[np.ndarray]) -> Tuple[int, int]:
        ...
        if stream_cleanup:
            cleaned_has_foreground = _cleanup_prediction_slice_inplace(
                ...
                fill_holes=bool(stream_fill_holes),
            )
...
def predict_in_memory_volume_and_submit_accumulation(
    ...
    *,
    ...
    return predict_source_and_submit_accumulation(
        ...
        streaming_cleanup_fill_holes=bool(streaming_cleanup_fill_holes),
...
def _cleanup_prediction_slice_inplace(
    ...
    *,
    ...
    v13.0.0 postprocessing order is confidence gate -> min-radius filter ->
    2D hole filling.  This function applies only slice-local semantics; cleanup
    that depends on a transposed/volume-level view, such as Sagittal or Coronal
    transverse-plane min-radius filtering, is deliberately deferred until the full
    view volume exists.
    """
    idx_i = int(idx)
    ...
    if bool(fill_holes) and np.any(mask_slice):
        if backend_norm == 'opencv':
...
def cleanup_view_volume_after_prediction_inplace(
    ...
    *,
    ...
    deferred_min_radius = _view_needs_deferred_volume_cleanup_after_streaming(view, float(min_radius))

    if bool(precleaned_slice_cleanup):
        # Single-angle streaming cleanup has already applied confidence gating and,
        # when valid for the view, min-radius filtering.  For Sagittal/Coronal,
        # streaming deliberately skipped hole filling so the deferred transverse-plane
        # min-radius filter can still run before hole filling, preserving v13 order.
        if bool(deferred_min_radius):
            ...
            fused_slice_cleanup_inplace(
                ...
                fill_holes=True,
                workers=int(workers),
                desc=f'2D hole fill after deferred min-radius ({view.name})',
            )
        flush_array(mask_mm)
        if confmap_mm is not None:
            flush_array(confmap_mm)
    ...
    if bool(deferred_min_radius):
        fused_slice_cleanup_inplace(
            ...
            fill_holes=False,
            workers=int(workers),
            desc=f'Confidence cleanup before deferred min-radius ({view.name})',
        )
        apply_view_min_radius_filter_inplace(
            mask_mm,
            view,
...
# --------------------------
# Scan-based interpolation helpers
# --------------------------
...
def _write_nrrd_ascii_header(
    ...
    *,
    ...
    lines: List[str] = [
        'NRRD0005',
        '# Complete NRRD file generated by GPT-5.5-Pro_v13.0.0_SLURM.py',
...
def main() -> None:
    ...
    device_plan = resolve_device_plan(args.device)
    resolved_retina_processor = resolve_retina_mask_processor_arg(args.retina_mask_processor, device_plan)
    set_retina_mask_processor(resolved_retina_processor)
    print(
        f'Inference devices resolved from --device={args.device!r}: {list(device_plan.inference_devices)}; '
        f'multi_gpu={bool(device_plan.multi_gpu)}; retina_mask_processor={retina_mask_processor()}'
    )
    ...
    materialized_cube_resize_applied = False
    if should_resize_to_processing_cube(input_processing_shape, legacy_cube_shape):
        processing_shape = legacy_cube_shape
        materialized_cube_resize_applied = True
        print(
            'v13.0.0 processing geometry: legacy materialized approximate-cube resize enabled by environment. '
        ...
        if preprocess_streaming_active:
            volume_rgb = resize_volume_to_processing_cube_gray8_streaming(
                input_volume_rgb,
                processing_shape,
                temp_dir / 'input_volume.materialized_cube.gray8.dat',
    ...
    elif processing_mode == 'virtual_cube':
        processing_shape = legacy_cube_shape
        volume_rgb = input_volume_rgb
        print(
            'v13.0.0 processing geometry: virtual approximate-cube working dimensions active. '
            f'decoded source shape (t,Y,X)=({input_T},{input_H},{input_W}) -> '
            f'virtual working shape (t,Y,X)={processing_shape}. '
            'No whole-volume cube is materialized; each active view reslices directly from the decoded volume.'
        )
    else:
        processing_shape = input_processing_shape
        volume_rgb = input_volume_rgb
        if processing_mode == 'cube' and legacy_cube_shape == input_processing_shape:
            print(f'v13.0.0 processing geometry: materialized cube mode requested, but input is already within 5% cube tolerance ({processing_shape}).')
        else:
            print(
                'v13.0.0 processing geometry: native decoded volume mode explicitly requested. '
                f'input/processing shape (t,Y,X)={processing_shape}; v13 virtual cube target would have been {legacy_cube_shape}.'
            )
    ...
    (temp_dir / 'processing_volume.meta.json').write_text(
        json.dumps({
            ...
            'cube_resize_applied': bool(materialized_cube_resize_applied),
            'virtual_cube_dimensions_applied': bool(processing_mode == 'virtual_cube' and tuple(int(x) for x in processing_shape) != tuple(int(x) for x in input_processing_shape)),
            'dtype': 'uint8',
            'channels': 1,
            'fps': fps,
    ...
    if preprocess_streaming_active:
        spec_notes.append(
            'v13.0.0 streaming preprocessing is active: decoded native slices become available as ffmpeg produces them. Transverse readers wait only for the needed decoded slice; stack-sampling view families wait for the completed decoded volume. Legacy cube resize, when explicitly enabled, still streams its output slices.'
        )
    else:
        spec_notes.append(
            'v13.0.0 streaming preprocessing is disabled by YOLO_TTA_STREAMING_PREPROCESS=0; decode finishes before inference scheduling begins, and legacy cube resize runs only when explicitly requested.'
        )
    if background_model_load_enabled():
        spec_notes.append(
    ...
    if bool(single_angle_streaming_cleanup_active):
        spec_notes.append(
            'v13.0.0 single-angle streaming cleanup is active: exactly one --angle value was supplied, so YOLO result slices are confidence-filtered, view-native min_radius-filtered, and then 2D-hole-filled as they stream in when all slice-local semantics are valid. Sagittal/Coronal defer hole filling until after their transverse-plane min_radius cleanup.'
        )
    else:
        spec_notes.append(
            'v13.0.0 single-angle streaming cleanup is inactive because multiple --angle values are being accumulated or YOLO_TTA_SINGLE_ANGLE_STREAMING_CLEANUP=0; view cleanup therefore runs after all angle volumes for a view have accumulated in min_conf -> min_radius -> hole-fill order.'
        )
    ...
    if (int(T), int(H), int(W)) != (int(input_T), int(input_H), int(input_W)):
        if processing_mode == 'virtual_cube':
            spec_notes.append(
                f'Virtual working dimensions derived to satisfy the v13 approximate-cube rule: '
                f'(t,Y,X)=({int(T)},{int(H)},{int(W)}) from decoded source '
                f'(t,Y,X)=({int(input_T)},{int(input_H)},{int(input_W)}). No cubic volume was materialized; '
                'view renderers map directly from decoded source coordinates into view-native prediction coordinates.'
            )
        else:
            spec_notes.append(
                f'Working volume resized to materialized approximate-cube processing geometry '
    ...
    if cpu_retina_masks_enabled():
        spec_notes.append(
            'retina_mask_processor=cpu: YOLO predict receives retina_masks=False; compact mask protos/coefficients/boxes/confidences are moved to CPU and bbox-ROI retina-quality binary masks are reconstructed in prediction-result workers.'
        )
    else:
        spec_notes.append(
            'retina_mask_processor=gpu: YOLO predict receives retina_masks=True and Ultralytics resolves retina masks on the active inference GPU. This is the default when --device lists multiple GPUs unless explicitly overridden.'
        )
    ...
    spec_notes.append(
        'Interpolation endpoint discovery uses the v13.0.0 per-slice connected-component scan backed by cached per-slice component tables. '
        ...
        'Skeletonization and skeleton output have been removed from the v13 pipeline.'
    )
    spec_notes.append(
        f'Processing volume mode={processing_mode}; materialized_cube_resize_applied={bool(materialized_cube_resize_applied)}; '
        f'virtual_cube_dimensions_applied={bool(processing_mode == "virtual_cube" and tuple(int(x) for x in processing_shape) != tuple(int(x) for x in input_processing_shape))}. '
        'v13.0.0 defaults to virtual approximate-cube geometry so t/X/Y dimensions remain within the 5% band without a whole-volume cubic interpolation pass. '
        f'Approximate cube target is {tuple(int(x) for x in legacy_cube_shape)}; set YOLO_TTA_PROCESSING_VOLUME_MODE=cube or YOLO_TTA_FORCE_PROCESSING_CUBE=1 only for legacy materialized-cube comparison. '
        f'If materialized cube resize is enabled, T-axis backend={_cube_t_axis_resize_backend()} and YOLO_TTA_CUBE_T_RESIZE_BACKEND=slice_exact restores the previous endpoint-aligned per-slice interpolation path.'
    )
    ...
    # v13.0.0 still accepts exactly one segmentation model path.  For multi-GPU
    # inference, load one YOLO instance per GPU and schedule prediction sources to
    # those single-device workers explicitly; do not pass a multi-GPU device list to
    # Ultralytics predict(), because it does not load-balance inference sources.
    inference_worker_models: List[Dict[str, object]] = []
    for worker_idx, device_token in enumerate(device_plan.inference_devices):
        if int(worker_idx) == 0:
            worker_model = yolo_model
        else:
            print(f'Loading additional YOLO worker model for device {device_token}: {model_name}')
            worker_model = load_ultralytics_model(model_paths[0], task='segment')
        inference_worker_models.append({
            'worker_id': int(worker_idx),
            'device': str(device_token),
            'yolo': worker_model,
        })

    # Scheduling/output structures remain keyed by the single model stem.
    yolo_models: List[Tuple[str, object]] = [(model_name, yolo_model)]
    yolo_by_model_name: Dict[str, object] = {model_name: yolo_model}
    all_yolo_runtime_models: List[object] = [worker['yolo'] for worker in inference_worker_models]

    pred_cfg = PredictConfig(
        imgsz=args.imgsz,
        conf=args.conf,
        device=str(device_plan.primary_device),
        half=bool(args.half),
        int8=bool(args.int8),
    ...
    spec_notes.append(
        f'Inference device scheduling: resolved devices={list(device_plan.inference_devices)}, multi_gpu={bool(device_plan.multi_gpu)}. '
        'When multiple GPUs are listed, the pipeline loads one YOLO instance per GPU and assigns full-frame/tile prediction sources to individual single-device workers; the multi-GPU list is never passed directly to Ultralytics predict().'
    )
    spec_notes.append(
    ...
    eager_gpu_input_staging_ahead_sources = (
        ...
        if (gpu_input_staging_enabled(pred_cfg) and not bool(device_plan.multi_gpu))
        else 0
    )
    ...
    multi_gpu_inference_active = bool(len(inference_worker_models) > 1 and not device_plan.is_cpu)
    inference_executor: Optional[ThreadPoolExecutor] = None
    available_inference_workers: deque[Dict[str, object]] = deque()
    active_inference_futures: Dict[Future, Dict[str, object]] = {}
    if bool(multi_gpu_inference_active):
        inference_executor = ThreadPoolExecutor(max_workers=len(inference_worker_models), thread_name_prefix='yolo-infer-gpu')
        available_inference_workers = deque(inference_worker_models)
        print(
            'Explicit multi-GPU scheduler: enabled; each worker owns one YOLO model and one device: '
            + ', '.join(f"worker{w['worker_id']}->{w['device']}" for w in inference_worker_models)
        )
    else:
        print(f'Explicit multi-GPU scheduler: inactive; using single inference device {device_plan.primary_device}.')
    ...
    def _worker_predict_cfg(worker: Dict[str, object]) -> PredictConfig:
        return make_predict_config_for_device(pred_cfg, str(worker['device']))

    def _run_multi_gpu_fullframe_inference(
        worker: Dict[str, object],
        view: ViewInfo,
        job: AugJob,
        prediction_ref: PredictionVolumeRef,
    ) -> Dict[str, object]:
        worker_cfg = _worker_predict_cfg(worker)
        yolo_obj = worker['yolo']
        try:
            if bool(async_prediction_accumulation_active):
                handle = predict_in_memory_volume_and_submit_accumulation(
                    ...
                    out_size=int(args.imgsz),
                    cfg=worker_cfg,
                    view_union_mm=baseline_union_by_model_view[(str(model_name), str(view.name))],
                    view_confmap_mm=baseline_confmap_by_model_view[(str(model_name), str(view.name))],
                    ...
                    streaming_cleanup_fill_holes=not _view_needs_deferred_volume_cleanup_after_streaming(view, float(args.min_radius)),
                ...
                return {
                    ...
                    'context': {
                        ...
                        'view': view,
                        'job': job,
                        # Do not offload asynchronously in multi-GPU mode; the worker model may
                        # already be running its next assigned source when result workers finish.
                        'yolo': None,
                    },
                }
    ...
    def _run_multi_gpu_tile_inference(
        worker: Dict[str, object],
        ...
        tile_mask_path = temp_dir / 'tile_volumes' / str(model_name_local) / view.name / f'{tile_job.tile_id}.u8.dat'
        tile_conf_path = temp_dir / 'tile_volumes' / str(model_name_local) / view.name / f'{tile_job.tile_id}.confmap.u8.dat'
        tile_mask_path.parent.mkdir(parents=True, exist_ok=True)
        tile_mask_mm = allocate_workspace_array(
            shape=tile_shape,
            dtype=np.uint8,
            path=tile_mask_path,
            desc=f'{model_name_local}/{view.name}/{tile_job.tile_id} raw tile volume',
            prefer_memory=True,
        )
        if float(args.min_conf) > 0.0:
            tile_conf_mm = allocate_workspace_array(
                ...
                dtype=np.uint8,
                path=tile_conf_path,
                desc=f'{model_name_local}/{view.name}/{tile_job.tile_id} raw tile confidence workspace',
                prefer_memory=True,
            )
        ...
        try:
            if bool(async_prediction_accumulation_active):
                ...
                return {
                    ...
                    'context': {
                        'kind': 'tile',
                        'model_name': str(model_name_local),
                        'view': view,
                        'tile_job': tile_job,
                        'tile_mask_mm': tile_mask_mm,
    ...
    def _submit_ready_multi_gpu_inference_jobs() -> None:
        if not bool(multi_gpu_inference_active) or inference_executor is None:
            return
        while available_inference_workers:
            if ready_fullframe:
                worker = available_inference_workers.popleft()
                view, job, prediction_ref = ready_fullframe.popleft()
                _ensure_baseline_workspaces(str(model_name), view)
                print(f"Inferencing full-frame source on {worker['device']}: {view.name}/{job.aug_id}")
                fut = inference_executor.submit(_run_multi_gpu_fullframe_inference, worker, view, job, prediction_ref)
                active_inference_futures[fut] = {'worker': worker, 'kind': 'fullframe'}
                continue

            if ready_tile_infer:
                model_name_local, view, tile_job, prediction_ref = ready_tile_infer.popleft()
                ready_key = (str(model_name_local), str(view.name), str(tile_job.tile_id))
                if ready_key in tile_inference_done:
                    close_prediction_volume_ref(prediction_ref, keep_temp=bool(keep_temp_artifacts))
                    continue
                worker = available_inference_workers.popleft()
                print(f"Inferencing tile source on {worker['device']}: {model_name_local}/{view.name}/{tile_job.tile_id}")
                fut = inference_executor.submit(_run_multi_gpu_tile_inference, worker, str(model_name_local), view, tile_job, prediction_ref)
                active_inference_futures[fut] = {'worker': worker, 'kind': 'tile'}
                continue
            break

    def _drain_completed_multi_gpu_inference_futures() -> None:
        for fut in list(active_inference_futures.keys()):
            if not fut.done():
                continue
            active_inference_futures.pop(fut, None)
            result = fut.result()
            worker = result.get('worker')
            if isinstance(worker, dict):
                available_inference_workers.append(worker)
            kind = str(result.get('kind', ''))

            if 'handle' in result:
                _submit_prediction_accumulation_join(result['handle'], dict(result.get('context', {})))
                continue

            if kind == 'fullframe':
                view = result['view']
                assert isinstance(view, ViewInfo)
                pred_stats = dict(result.get('pred_stats', {}))
                view_prediction_stats[str(view.summary_family)] = int(view_prediction_stats.get(str(view.summary_family), 0)) + int(pred_stats.get('prediction_count', 0))
                remaining_key = (str(result.get('model_name', model_name)), view.name)
                fullframe_remaining[remaining_key] = int(fullframe_remaining.get(remaining_key, 0)) - 1
                if int(fullframe_remaining.get(remaining_key, 0)) == 0:
                    _submit_view_prepare(str(remaining_key[0]), view)
                continue

            if kind == 'tile':
                model_name_local = str(result['model_name'])
                view = result['view']
                tile_job = result['tile_job']
                assert isinstance(view, ViewInfo)
                assert isinstance(tile_job, DenseTileJob)
                ready_key = (str(model_name_local), str(view.name), str(tile_job.tile_id))
                tile_inference_done.add(ready_key)
                ...
                if bool(result.get('empty', False)):
                    _mark_tile_staged(str(model_name_local), str(view.name), str(tile_job.config_id), str(tile_job.tile_id))
                    continue
                task = result.get('task')
                if not isinstance(task, TilePostprocessTask):
                    raise RuntimeError('Multi-GPU tile inference returned no TilePostprocessTask for a non-empty tile')
                tile_fut = tile_postprocess_executor.submit(
                    postprocess_tile_volume_after_inference,
                    task,
                    view=view,
                ...
                tile_cleanup_futures[tile_fut] = (str(model_name_local), str(view.name), str(tile_job.config_id), str(tile_job.tile_id))
                continue

            raise RuntimeError(f'Unknown multi-GPU inference result kind: {kind!r}')
    ...
    def _log_scheduler_wait_state(force: bool = False) -> None:
        ...
        print(
            ...
            f'active_inference={len(active_inference_futures)}, '
    ...
    try:
        ...
        while True:
            ...
            _drain_completed_multi_gpu_inference_futures()
            _drain_completed_prediction_accumulation_futures()
            _drain_completed_background_futures()
            _pump_prediction_volume_build_queue()
            _warmup_ready_prediction_sources()
            _submit_ready_multi_gpu_inference_jobs()

            if (not bool(multi_gpu_inference_active)) and ready_fullframe:
                view, job, prediction_ref = ready_fullframe.popleft()
            ...
            if (not bool(multi_gpu_inference_active)) and ready_tile_infer:
            ...
            waitables.extend(list(active_inference_futures.keys()))
            ...
            if not waitables:
                ...
                if (
                    ...
                    not active_inference_futures and
    ...
    for mm in baseline_confmap_by_model_view.values():
        close_memmap_array(mm)
    for yolo in all_yolo_runtime_models:
