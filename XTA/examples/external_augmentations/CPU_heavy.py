"""CPU companion to the heavy XTA PTA GPU augmentation policy.

The policy preserves the heavy transform graph, parameter ranges, affine
...
class CPUAugmentation:
    ...
    def _elastic_displacement(seed: int, height: int, width: int) -> np.ndarray:
        ...
        return np.ascontiguousarray(displacement * 27.5, dtype=np.float32)
