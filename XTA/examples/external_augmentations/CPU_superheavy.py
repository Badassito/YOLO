"""CPU companion to the superheavy XTA PTA GPU augmentation policy.

The policy preserves the superheavy transform graph, parameter ranges, affine
...
class CPUAugmentation:
    ...
    def _elastic_displacement(seed: int, height: int, width: int) -> np.ndarray:
        ...
        return np.ascontiguousarray(displacement * 35.0, dtype=np.float32)
