...
def decode_video_to_memmap_rgb24(
    ...
    try:
        ...
        if proc.returncode not in (0, None, -9):
            msg = err_bytes.decode("utf-8", errors="ignore") if err_bytes else "Unknown error"
...
def close_ffmpeg_writer(proc: subprocess.Popen) -> None:
    # Delegate cleanup and pipe closing to communicate natively
    # This safely prevents "ValueError: flush of closed file"
...
class AffineSpec:
    ...
    pad_off_x: float
    pad_off_y: float
    M_fwd: np.ndarray  # 2x3 float32
    M_inv: np.ndarray  # 2x3 float32
...
def build_affine(
    ...
    """
    Constructs a Native -> Augmented affine matrix mapping mathematically.
    """
    ...
    scale = out_size / float(P)

    # Since the center of the source perfectly aligns with the center of the padded
    # canvas, we can rotate and scale around the source center to calculate the mapping.
    cx_s = (src_w - 1) / 2.0
    cy_s = (src_h - 1) / 2.0
    cx_d = (out_size - 1) / 2.0
    cy_d = (out_size - 1) / 2.0

    M_fwd = cv2.getRotationMatrix2D((cx_s, cy_s), angle_deg, scale)
    M_fwd[0, 2] += (cx_d + shift_dx) - cx_s
    M_fwd[1, 2] += (cy_d + shift_dy) - cy_s
...
def predict_video_and_accumulate(
    ...
    M_fwd: np.ndarray,                 # 2x3, maps Native->Aug for warpAffine inverse mapping
    native_h: int,
    native_w: int,
) -> None:
    """
    ...
    for idx, r in enumerate(results):
        ...
        # Ensure union mask matches expected size
        ...
        # Undo augmentation (warp mask from Aug back to Native coordinate space).
        # By providing M_fwd (Native -> Aug) and using WARP_INVERSE_MAP,
        # OpenCV correctly utilizes it as the exact Destination -> Source mapping.
                ...
                if not aug_video.exists():
                    ...
                    try:
                        ...
                        for frame in tqdm(iter_view_frames(volume_rgb, view), total=view.num_slices, desc=f"Augment {view.name} {aug_id}"):
                            # Forward Warping: Mapping Native (Source) to Aug (Destination).
                            # warpAffine receives the Source->Dest mapping matrix and handles the internal inversion.
                            out = cv2.warpAffine(
                                frame,
                                aff.M_fwd,
                    ...
                    aug_meta.write_text(json.dumps({
                        ...
                        "M_fwd": aff.M_fwd.tolist(),
                        "M_inv": aff.M_inv.tolist(),
                    }, indent=2))
