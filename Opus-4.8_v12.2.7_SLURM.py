...
This v12.2.7_SLURM implementation of the v12.2.0_SLURM specification plus v12.2.1-v12.2.7 performance patches:
  - v12.2.7 fixes the "GPU idle between views" stall: per-view full-frame postprocess + interpolation
    (prepare_view_volume_after_fullframe) is pure-Python/GIL-bound, so running it on a background
    ThreadPoolExecutor (v12.2.6) serialized it against the cv2 prediction-volume builder threads that
    keep the GPU fed; while a view interpolated, the bounded prefetch queue drained and the inference
    loop parked in wait(), leaving the GPU at ~0-6% for minutes. v12.2.7 runs that stage in separate
    forkserver processes so it no longer holds the parent interpreter's GIL. Volumes are exchanged by
    path (file-backed memmap on NVMe scratch): single-node mmap(MAP_SHARED) shares the OS page cache, so
    in-place writes are visible across processes without pickling multi-GB arrays or msync on the hot
    path. Set YOLO_TTA_INTERP_BACKEND=thread to restore the v12.2.6 in-thread behavior; tune the pool
    with YOLO_TTA_INTERP_PROCESSES.
...
def predict_source_and_accumulate(
    ...
    *,
    ...
    # v12.2.7: under the process-isolated interpolation backend, the union/confmap are file-backed
    # memmaps shared with the forkserver child via the OS page cache, which is coherent on a single
    # node WITHOUT msync. The per-prediction flush would msync tens of GB to NVMe on the GPU-driving
    # main thread (a fresh "between views" stall), so skip it there; scratch durability is not
    # required. The default (thread) backend keeps the original flush behavior.
    if not interp_process_backend_enabled():
    ...
    if bool(dense_tiling_active):
        if _view_uses_interpolation(view, int(interpolate)):
            ...
            final_view_volume = copy_workspace_array(
                ...
                # v12.2.7: under the process backend this copy must reach the parent by path,
                # so write it straight to disk (the wrapper then references it with no second
                # copy). The thread backend keeps the original in-RAM copy.
                prefer_memory=not interp_process_backend_enabled(),
                workers=int(slice_workers),
            )
        else:
...
# ============================================================================
# v12.2.7: process-isolated background full-frame postprocess + interpolation
# ----------------------------------------------------------------------------
# prepare_view_volume_after_fullframe() is the dominant "between views" CPU
# stage and is pure-Python/GIL-bound (endpoint seed planning, candidate search,
# linear bridge plans).  Running it on a ThreadPoolExecutor cannot overlap with
# the GPU because the GIL serializes it against the cv2 prediction-volume builder
# threads.  The helpers below run it in a forkserver process pool instead, so it
# stops contending for the parent interpreter's GIL and the builders keep the
# inference queue full.
#
# Data exchange is by *path*, not by pickling arrays: the view's union/confmap
# are file-backed memmaps on NVMe scratch, and on a single SLURM node mmap of the
# same file shares the OS page cache, so in-place writes are visible across
# processes with no msync.  The child returns small path/stat results and the
# parent re-opens them into an ordinary PreparedViewResult, leaving the existing
# scheduler/consumer code unchanged.  forkserver (not fork) is required so the
# workers are clean and single-threaded, avoiding fork-from-a-multithreaded-parent
# deadlocks and never inheriting CUDA state.
# ============================================================================

_INTERP_BACKEND_CACHE: Dict[str, bool] = {}


def interp_process_backend_enabled() -> bool:
    """True when per-view interpolation should run in a forkserver process pool.

    Controlled by YOLO_TTA_INTERP_BACKEND (default 'process'); falls back to the
    in-thread path when forkserver is unavailable (e.g. Windows) so the script is
    still runnable everywhere.
    """
    cached = _INTERP_BACKEND_CACHE.get('enabled')
    if cached is not None:
        return bool(cached)
    backend = os.environ.get('YOLO_TTA_INTERP_BACKEND', 'process').strip().lower()
    ...
    if backend == 'process':
        try:
            ...
            enabled = 'forkserver' in set(_mp.get_all_start_methods())
        except Exception:
            enabled = False
    _INTERP_BACKEND_CACHE['enabled'] = bool(enabled)
    return bool(enabled)


def _interp_mp_context():
    import multiprocessing as _mp
    return _mp.get_context('forkserver')


def _interp_pool_warmup() -> int:
    """Trivial task used to pre-start the forkserver server + first worker while the
    parent is still effectively single-threaded (see the warmup call in main)."""
    return 1


def _memmap_backing_filename(arr: object) -> Optional[str]:
...
def _persist_array_for_ipc(arr: np.ndarray, fallback_path: Path, *, workers: int = 1) -> Tuple[str, Tuple[int, ...]]:
    """Return (path, shape) for an array the parent must read after the child exits.

    A file-backed memmap is already shareable, so its filename is returned directly.
    An in-memory array (e.g. a dense-tiling frozen full-frame copy) is written once to
    ``fallback_path`` on NVMe scratch and that path is returned.
    """
    shape = tuple(int(x) for x in arr.shape)
    fn = _memmap_backing_filename(arr)
    if fn is not None:
        return str(fn), shape
    fallback_path = Path(fallback_path)
    fallback_path.parent.mkdir(parents=True, exist_ok=True)
    dst = copy_workspace_array(
        np.asarray(arr),
        fallback_path,
        desc=f'IPC persist {fallback_path.name}',
        prefer_memory=False,
        workers=int(workers),
    )
    flush_array(dst)
    close_memmap_array(dst)
    return str(fallback_path), shape


@dataclass
class _PreparedViewPayload:
    """Picklable, path-based result returned from the interpolation subprocess."""
    model_name: str
    view_name: str
    native_support_path: str
    native_support_shape: Tuple[int, int, int]
    final_view_volume: Optional[str]  # None | 'same' (== native) | path
    parent_mask_support_root: Optional[str]
    parent_bridge_support_root: Optional[str]
    interpolation_stats: List[Dict[str, object]]
    nrrd_layers: List[NrrdLayerRef]


def _prepare_view_volume_after_fullframe_subprocess(
    *,
    model_name: str,
    view: ViewInfo,
    union_path: str,
    union_shape: Tuple[int, int, int],
    confmap_path: Optional[str],
    confmap_shape: Optional[Tuple[int, int, int]],
    temp_dir: str,
    dense_tiling_active: bool,
    min_conf: float,
    min_radius: float,
...
) -> _PreparedViewPayload:
    """forkserver entrypoint: open the view's union/confmap by path, run the existing
    in-place postprocess + interpolation, and return small path-based results."""
    temp_dir_p = Path(temp_dir)
    union_mm = np.memmap(
        union_path, dtype=np.uint8, mode='r+', shape=tuple(int(x) for x in union_shape)
    )
    confmap_mm = None
    if confmap_path is not None and confmap_shape is not None:
        confmap_mm = np.memmap(
            confmap_path, dtype=np.uint8, mode='r+', shape=tuple(int(x) for x in confmap_shape)
        )

    result = prepare_view_volume_after_fullframe(
        model_name=str(model_name),
        view=view,
        union_mm=union_mm,
        confmap_mm=confmap_mm,
        union_path=Path(union_path),
        confmap_path=(Path(confmap_path) if confmap_path is not None else None),
        temp_dir=temp_dir_p,
        ...
        interpolation_walk_back=int(interpolation_walk_back),
        interpolation_candidates=int(interpolation_candidates),
        interpolate_passes=int(interpolate_passes),
        interpolate_min_radius=float(interpolate_min_radius),
        interpolation_search_angle=float(interpolation_search_angle),
        keep_temp=bool(keep_temp),
        slice_workers=int(slice_workers),
        interpolation_task_workers=int(interpolation_task_workers),
        nrrd_layers_enabled=bool(nrrd_layers_enabled),
    )

    ipc_dir = temp_dir_p / 'ipc' / str(model_name)
    native = result.native_support_mm
    native_path, native_shape = _persist_array_for_ipc(
        native, ipc_dir / f'{view.name}.native_support.u8.dat', workers=int(slice_workers)
    )

    final = result.final_view_volume_mm
    if final is None:
        final_payload: Optional[str] = None
    elif final is native:
        final_payload = 'same'
    else:
        final_path, _ = _persist_array_for_ipc(
            final, ipc_dir / f'{view.name}.final_view.u8.dat', workers=int(slice_workers)
        )
        final_payload = final_path
        close_memmap_array(final)  # safe no-op for in-memory arrays

    parent_mask_root = (
        str(getattr(result.parent_mask_support_mm, 'root'))
        if result.parent_mask_support_mm is not None else None
    )
    parent_bridge_root = (
        str(getattr(result.parent_bridge_support_mm, 'root'))
        if result.parent_bridge_support_mm is not None else None
    )

    # Page cache is coherent on a single node, but flush the native volume once here so
    # the result is durable if scratch is later inspected; this is off the GPU hot path.
    flush_array(native)
    close_memmap_array(native)
    if confmap_mm is not None:
        close_memmap_array(confmap_mm)

    return _PreparedViewPayload(
        model_name=str(result.model_name),
        view_name=str(result.view_name),
        native_support_path=str(native_path),
        native_support_shape=(int(native_shape[0]), int(native_shape[1]), int(native_shape[2])),
        final_view_volume=final_payload,
        parent_mask_support_root=parent_mask_root,
        parent_bridge_support_root=parent_bridge_root,
        interpolation_stats=list(result.interpolation_stats),
        nrrd_layers=list(result.nrrd_layers),
    )


def _reopen_prepared_view_payload(payload: '_PreparedViewPayload') -> PreparedViewResult:
    """Parent-side: rebuild a PreparedViewResult by re-opening the child's outputs."""
    native = np.memmap(
        payload.native_support_path,
        dtype=np.uint8,
        mode='r+',
        shape=tuple(int(x) for x in payload.native_support_shape),
    )
    if payload.final_view_volume is None:
        final: Optional[np.ndarray] = None
    elif payload.final_view_volume == 'same':
        final = native
    else:
        final = np.memmap(
            payload.final_view_volume,
            dtype=np.uint8,
            mode='r+',
            shape=tuple(int(x) for x in payload.native_support_shape),
        )
    parent_mask = (
        RawBBoxMaskStore.open(Path(payload.parent_mask_support_root))
        if payload.parent_mask_support_root else None
    )
    parent_bridge = (
        RawBBoxMaskStore.open(Path(payload.parent_bridge_support_root))
        if payload.parent_bridge_support_root else None
    )
    return PreparedViewResult(
        model_name=str(payload.model_name),
        view_name=str(payload.view_name),
        native_support_mm=native,
        final_view_volume_mm=final,
        interpolation_stats=list(payload.interpolation_stats),
        nrrd_layers=list(payload.nrrd_layers),
        parent_mask_support_mm=parent_mask,
        parent_bridge_support_mm=parent_bridge,
    )
...
def main() -> None:
    ...
    # v12.2.7: decide whether per-view full-frame interpolation runs in forkserver
    # processes (default) so its pure-Python/GIL-bound work stops starving the cv2
    # prediction-volume builders that keep the GPU fed.
    interp_uses_processes = interp_process_backend_enabled()
    child_interp_slice_workers = int(slice_postprocess_workers)
    child_interp_task_workers = int(parent_interpolation_task_workers)
    interp_process_workers = 0
    if interp_uses_processes:
        default_interp_processes = max(1, min(int(parent_postprocess_workers), int(parent_interpolation_overlap)))
        interp_process_workers = max(1, _env_int('YOLO_TTA_INTERP_PROCESSES', int(default_interp_processes)))
        # Partition the CPU budget across the concurrent interpolation processes so the
        # forkserver children together respect the same ~2x oversubscription target rather
        # than each spawning the full per-parent worker count.
        child_interp_slice_workers = max(1, int(slice_postprocess_workers) // int(interp_process_workers))
        child_interp_task_workers = max(1, int(parent_interpolation_task_workers) // int(interp_process_workers))
    ...
    if interp_uses_processes:
        print(
            'Full-frame interpolation backend: forkserver PROCESS pool '
            f'(processes: {interp_process_workers}, per-child slice workers: {child_interp_slice_workers}, '
            f'per-child interpolation workers: {child_interp_task_workers}); '
            'union/confmap are file-backed memmaps shared by path. '
            'Set YOLO_TTA_INTERP_BACKEND=thread to disable.'
        )
    else:
        print(
            'Full-frame interpolation backend: in-thread (v12.2.6 behavior). '
            'GPU may idle between views while pure-Python interpolation holds the GIL; '
            'set YOLO_TTA_INTERP_BACKEND=process on Linux/SLURM to overlap it with inference.'
        )
    print(
    ...
    for view in inference_views:
        for model_name, _ in yolo_models:
            ...
            baseline_union_by_model_view[(model_name, view.name)] = allocate_workspace_array(
                ...
                # v12.2.7: the process interpolation backend shares this volume with a
                # forkserver child by path, so it must be a file-backed memmap (the page
                # cache keeps it RAM-resident on a single node). The thread backend keeps
                # the original in-RAM allocation.
                prefer_memory=not interp_uses_processes,
            )
            if float(args.min_conf) > 0.0:
                baseline_confmap_by_model_view[(model_name, view.name)] = allocate_workspace_array(
                    ...
                    # v12.2.7: file-backed for cross-process sharing under the process backend.
                    prefer_memory=not interp_uses_processes,
                )
    ...
    # v12.2.7: forkserver pool that runs prepare_view_volume_after_fullframe off the parent
    # interpreter's GIL so the cv2 prediction-volume builders keep the GPU fed during interpolation.
    parent_postprocess_process_executor: Optional[ProcessPoolExecutor] = None
    if interp_uses_processes:
        try:
            parent_postprocess_process_executor = ProcessPoolExecutor(
                max_workers=int(interp_process_workers),
                mp_context=_interp_mp_context(),
            )
            # Force the forkserver server + first worker to start NOW. The prediction-volume,
            # parent-postprocess and tile-postprocess thread pools above have not yet spawned
            # any threads (they spawn lazily on first submit, which happens later in the loop),
            # so the parent is still effectively single-threaded here. All forkserver workers
            # are forked from this server, never from the busy parent, which avoids inheriting a
            # locked malloc arena or the parent's CUDA context. Children never import torch.
            parent_postprocess_process_executor.submit(_interp_pool_warmup).result()
        except Exception as warmup_exc:  # pragma: no cover - environment dependent
            print(
                f'v12.2.7: interpolation process pool unavailable ({warmup_exc!r}); '
                'falling back to the in-thread interpolation backend for this run.'
            )
            if parent_postprocess_process_executor is not None:
                try:
                    parent_postprocess_process_executor.shutdown(wait=False, cancel_futures=True)
                except Exception:
                    pass
            parent_postprocess_process_executor = None
            interp_uses_processes = False
    ...
    def _submit_view_prepare(model_name: str, view: ViewInfo) -> None:
        ...
        if parent_postprocess_process_executor is not None:
            # v12.2.7 process backend: hand the union/confmap to a forkserver child by path.
            # The parent's memmaps are dropped (not closed-with-msync) so we don't block the
            # GPU thread; the child maps the same files and shares the page cache on this node.
            union_shape = (int(view.num_slices), int(view.src_h), int(view.src_w))
            confmap_shape = union_shape if confmap_path is not None else None
            union_mm = None
            confmap_mm = None
            fut = parent_postprocess_process_executor.submit(
                _prepare_view_volume_after_fullframe_subprocess,
                model_name=str(model_name),
                view=view,
                union_path=str(union_path),
                union_shape=union_shape,
                confmap_path=(str(confmap_path) if confmap_path is not None else None),
                confmap_shape=confmap_shape,
                ...
                slice_workers=int(child_interp_slice_workers),
                interpolation_task_workers=int(child_interp_task_workers),
    ...
    def _drain_completed_background_futures() -> None:
        for fut in list(view_processing_futures.keys()):
            if not fut.done():
                continue
            result = fut.result()
            if isinstance(result, _PreparedViewPayload):
                # v12.2.7 process backend: re-open the child's path-based outputs as memmaps
                # so the rest of the scheduler sees an ordinary PreparedViewResult.
                result = _reopen_prepared_view_payload(result)
    ...
    try:
        ...
        if parent_postprocess_process_executor is not None:
            parent_postprocess_process_executor.shutdown(wait=True)
